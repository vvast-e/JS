(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function e(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(i){if(i.ep)return;i.ep=!0;const s=e(i);fetch(i.href,s)}})();class h{async get(t){try{const e=await fetch(t);if(!e.ok)throw new Error(`Ошибка GET-запроса: ${e.status}`);return await e.json()}catch(e){throw console.error("Ошибка сети или сервера:",e),e}}async post(t,e){try{const r=await fetch(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!r.ok)throw new Error(`Ошибка POST-запроса: ${r.status}`);const i=r.headers.get("content-type");let s={};return i&&i.includes("application/json")&&(s=await r.json()),s}catch(r){throw console.error("Ошибка отправки данных:",r),r}}async put(t,e){try{const r=await fetch(t,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!r.ok)throw new Error(`Ошибка PUT-запроса: ${r.status}`);return await r.json()}catch(r){throw console.error("Ошибка редактирования:",r),r}}async delete(t){try{const e=await fetch(t,{method:"DELETE"});if(!e.ok)throw new Error(`Ошибка DELETE-запроса: ${e.status}`);const r=e.headers.get("content-type");return r&&r.includes("application/json")?await e.json():{}}catch(e){throw console.error("Ошибка при удалении:",e),e}}}const n=new h;class p{constructor(){this.baseUrl="http://localhost:3000"}getStocks(){return`${this.baseUrl}/stocks`}getStockById(t){return`${this.baseUrl}/stocks/${t}`}createStock(){return`${this.baseUrl}/stocks`}removeStockById(t){return`${this.baseUrl}/stocks/${t}`}updateStockById(t){return`${this.baseUrl}/stocks/${t}`}replaceStockById(t){return`${this.baseUrl}/stocks/${t}`}}const c=new p;class d{constructor(t){this.parent=t}getHTML(t){return`
        ...
        `}addListeners(t,e){const r=this.parent.querySelector(`#click-card-${t.id}`);r?r.addEventListener("click",e):console.error(`Кнопка click-card-${t.id} не найдена`)}render(t,e){const r=this.getHTML(t);this.parent.insertAdjacentHTML("beforeend",r),this.addListeners(t,e)}}class u{constructor(t,e={}){this.parent=t,this.options=e}getHTML(){return`
        ...
        `}render(t,e){let r;t.title==="Ориентал"?r=[{title:"Особенности характера",content:`
                    <ul>
                ...
                `},{title:"Особенности ухода",content:`<ul>
                    ...
                    </ul>`}]:t.title==="Абиссин"?r=[{title:"Особенности породы",content:`
                    <ul>
                ...
                `},{title:"Особенности ухода",content:`<ul>
                    ...
                    </ul>`}]:r=[{title:"Возможные заболевания",content:`
                    <ul>
                ...
                `},{title:"Особенности ухода",content:`<ul>
                        ...
                        <li>Вакцинация. Проводите своевременную дегельминтизацию и прививки.</li>
                    </ul>`}];const i=new m(this.parent,r),s=this.getHTML(t,i);this.parent.insertAdjacentHTML("beforeend",s);const o={title:"Интересный факт",message:"Ваш интересный факт здесь.",time:"сейчас",autohide:!0,delay:5e3};new u(this.parent,o).render()}}class b{constructor(t){this.parent=t}addListeners(t){document.getElementById("back-button").addEventListener("click",t)}getHTML(){return`
            <button id="back-button" class="btn btn-primary" type="button">Назад</button>
        `}render(t){const e=this.getHTML();this.parent.insertAdjacentHTML("beforeend",e),this.addListeners(t)}}class f{constructor(t,e,r,i){this.parent=t,this.stockId=e,this.initialData=r,this.onSubmit=i}render(){this.parent.innerHTML=`
        ...
        `,this.parent.querySelector(".edit-form").addEventListener("submit",this.handleSubmit.bind(this))}async handleSubmit(t){t.preventDefault();const e={title:t.target.title.value.trim(),text:t.target.text.value.trim(),src:t.target.src.value.trim()};try{const r=await n.put(c.replaceStockById(this.stockId),e);alert("Данные успешно обновлены!"),this.onSubmit(r)}catch(r){alert("Ошибка при сохранении изменений"),console.error("Ошибка PUT-запроса:",r)}}}class y{constructor(t,e,r){this.parent=t,this.id=e,this.mainPage=r,this.isEditing=!1,this.pageRoot=null}async getData(){try{const t=await n.get(c.getStockById(this.id));this.renderData(t)}catch(t){console.error("Ошибка загрузки карточки:",t)}}async handleDelete(){if(confirm("Вы уверены, что хотите удалить эту карточку?"))try{await n.delete(c.removeStockById(this.id)),alert("Карточка успешно удалена"),this.mainPage.getData(),this.clickBack()}catch(t){alert("Ошибка при удалении карточки"),console.error("Ошибка DELETE-запроса:",t)}}clickBack(){this.mainPage.render()}toggleEditMode(){if(this.isEditing=!this.isEditing,this.isEditing)this.renderEditForm();else{const t=this.pageRoot.querySelector(".edit-form-container");t&&t.remove()}}renderEditForm(){const t=document.createElement("div");t.className="edit-form-container",this.pageRoot.appendChild(t),new f(t,this.id,this.productData,async e=>{try{const r=await n.put(c.replaceStockById(this.id),e);this.handleFormSubmit(r),this.toggleEditMode()}catch(r){alert("Ошибка при сохранении изменений"),console.error("Ошибка редактирования:",r)}}).render()}handleFormSubmit(t){this.productData=t,this.renderData(t)}renderData(t){if(!this.pageRoot||!t)return;this.productData=t,this.pageRoot.innerHTML="";const e=document.createElement("div");this.pageRoot.appendChild(e),new g(e).render(t,this.mainPage);const i=document.createElement("button");i.textContent="Редактировать",i.className="edit-button",i.addEventListener("click",()=>this.toggleEditMode()),this.pageRoot.appendChild(i);const s=document.createElement("button");s.textContent="Удалить",s.className="delete-button btn btn-danger ms-3",s.addEventListener("click",()=>this.handleDelete()),this.pageRoot.appendChild(s),new b(this.pageRoot).render(this.clickBack.bind(this))}render(){this.parent.innerHTML="",this.pageRoot=document.createElement("div"),this.parent.appendChild(this.pageRoot),this.getData()}}class v{constructor(t,e){this.parent=t,this.onCreate=e}render(){this.parent.innerHTML=`
        ...
        `,this.parent.querySelector(".create-form").addEventListener("submit",this.handleSubmit.bind(this))}async handleSubmit(t){t.preventDefault();const e={title:t.target.title.value.trim(),text:t.target.text.value.trim(),src:t.target.src.value.trim()};try{const r=await n.post(c.createStock(),e);alert("Карточка успешно создана!"),this.onCreate(r)}catch(r){alert("Ошибка при создании карточки"),console.error("Ошибка создания карточки:",r)}}}class w{constructor(t){this.parent=t,this.container=t,this.pageRoot=document.createElement("div"),this.pageRoot.className="main-page"}async getData(){try{return await n.get(c.getStocks())}catch(t){throw console.error("Ошибка получения данных:",t),t}}async render(){this.parent.innerHTML="",this.parent.appendChild(this.pageRoot),this.pageRoot.innerHTML=this.getHTML();try{const t=await n.get(c.getStocks());this.renderData(t)}catch(t){this.showError(`Не удалось загрузить данные: ${t.message}`)}}async renderData(t){if(!t||!Array.isArray(t)){console.error("Получены некорректные данные:",t),this.showError("Невозможно отобразить карточки");return}this.pageRoot.innerHTML="";const e=this.pageRoot.querySelector(".main-header");this.pageRoot.innerHTML="",e&&this.pageRoot.appendChild(e);const r=document.createElement("div");r.className="cards-container",r.style.display="flex",r.style.flexWrap="wrap",r.style.gap="20px",t.forEach(s=>{if(!s||!s.id){console.warn("Пропущен невалидный элемент:",s);return}try{const o=document.createElement("div");o.className="card-wrapper",new d(o).render(s,()=>this.clickCard(s.id)),r.appendChild(o)}catch(o){console.error("Ошибка рендеринга карточки:",o)}});const i=document.createElement("div");new v(i,async s=>{try{const o=document.createElement("div");new d(o).render(s,()=>this.clickCard(s.id)),r.appendChild(o)}catch(o){console.error("Ошибка при добавлении новой карточки:",o)}}).render(),this.pageRoot.prepend(i),this.pageRoot.appendChild(r)}clickCard(t){new y(this.container,t,this).render()}showError(t){const e=document.createElement("div");e.className="error-message",e.textContent=t,this.pageRoot.innerHTML="",this.pageRoot.appendChild(e)}getHTML(){return`
